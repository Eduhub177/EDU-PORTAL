...
function TickerBar() {
  const { data: results } = useCollection<TickerResult>("results");
  ...
  const items = [...topResults, ...topResults];

  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-md border-t border-border/40 py-2 overflow-hidden">
      <div
      ...
      >
        {items.map((r, i) => (
          <span key={i} className="inline-flex items-center gap-2 px-6 text-sm shrink-0">
            ...
            <span className="text-muted-foreground">
              in {r.subject} · Class {r.studentClass}
            </span>
...
export default function Landing() {
  return (
    <PageTransition>
      <div className="flex flex-col min-h-[100dvh] relative overflow-hidden pb-10">
        <AnimatedBg />
        <Particles count={42} />
        ...
        <main className="flex-1 container mx-auto px-4 pt-8 pb-16 flex flex-col items-center justify-center text-center relative z-10">
          ...
          >
            Premium experience for teachers to create exams and students to
            compete. Real-time leaderboards, detailed analytics, and beautiful
            question interfaces.
          </motion.p>
